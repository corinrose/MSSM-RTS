...
class button():
	def __init__(self, name, pos, onClick, images, font=None, text=None, fontSize=48):
		self.name = name
		self.pos = pos
		self.onClick = onClick
		self.images = images #First = idle, second = mouseover, third = mousedown, forth=disabled
		self.size = [self.images[0].get_width(), self.images[0].get_height()]
		self.state = 0 #0 = idle, 1 = mouseover, 2 = mousedown, 3=disabled
		self.enabled = True
		self.drawText = False
		if text!=None:
			self.text = text
